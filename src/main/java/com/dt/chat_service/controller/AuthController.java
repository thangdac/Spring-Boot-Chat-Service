...
public class AuthController {
    ...
    public ResponseEntity<APIResponse<TokenResponse>> login(
        ...
        return ResponseEntity.ok(
                APIResponse.<TokenResponse>builder().result(tokens).build());
    }
    ...
    public ResponseEntity<APIResponse<Void>> register(@RequestBody @Valid UserCreationRequest request) {
        ...
        return ResponseEntity.ok(
                APIResponse.<Void>builder().message("Registration successful").build());
    }
    ...
    public ResponseEntity<APIResponse<Void>> logout(
            @RequestHeader("Authorization") String authHeader, @RequestBody @Valid RefreshRequest request) {
        String accessToken = authHeader.substring(7);
        authService.logout(accessToken, request.getRefreshToken());
        return ResponseEntity.ok(
                APIResponse.<Void>builder().message("Logout successful").build());
    }
    ...
    public ResponseEntity<APIResponse<IntrospectResponse>> introspect(
        ...
        return ResponseEntity.ok(
                APIResponse.<IntrospectResponse>builder().result(result).build());
    }
}
