...
public class ConversationService {
    ...
    public ConversationResponse createConversation(CreateConversationRequest request, UUID creatorId) {
        ...
        if (request.getType() == ConversationType.DIRECT) {
            ...
            Optional<Conversation> existing = conversationRepository.findDirectConversation(creatorId, targetId);
        ...
        conversation.getParticipants().add(buildParticipant(conversation, creatorId, true));

        for (UUID participantId : request.getParticipantIds()) {
            if (!participantId.equals(creatorId)) {
                conversation.getParticipants().add(buildParticipant(conversation, participantId, false));
            }
        }
    ...
    private ConversationParticipant buildParticipant(Conversation conversation, UUID userId, boolean isAdmin) {
